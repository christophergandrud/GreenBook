...
vars <- c("Quarter", "ElectionPeriod", "pres_party", "error.prop.deflator.q0", 
          ...
          "recession", "DebtGDP", "ExpenditureGDP",
...
CPIMatchedParty02 <- matchit(pres_party ~ recession + time_to_election + ElectionPeriod + ExpenditureGDP + PotentialGDP + GlobalModel + DiscountRate1qChange + DiscountRate2qChange + UNRATE + pres_party*ElectionPeriod, data = CPIEstimates02, method = "genetic", pop.size = 161)
...
# Party, Only pres*ElectionPeriod Interaction Quarters 0 to 2
CPIMatchedParty35 <- matchit(pres_party ~ recession + time_to_election + ElectionPeriod + ExpenditureGDP + PotentialGDP + GlobalModel + DiscountRate3qChange + DiscountRate4qChange + UNRATE + DiscountRate5qChange + pres_party*ElectionPeriod, data = CPIEstimates35, method = "genetic", pop.size = 161)
...
PL.02.0 <- zelig(error.prop.deflator.q0 ~ pres_party + time_to_election + recession + DebtGDP + ExpenditureGDP + PotentialGDP  + UNRATE + GlobalModel, model = "ls", data = CPIMdfParty02, cite = FALSE)

PL.02.1 <- zelig(error.prop.deflator.q1 ~ pres_party + time_to_election + recession + DebtGDP + ExpenditureGDP + PotentialGDP + DiscountRate1qChange + UNRATE + GlobalModel, model = "ls", data = subset(CPIMdfParty02, time_to_election != 15), cite = FALSE)

PL.02.2 <- zelig(error.prop.deflator.q2 ~ pres_party + time_to_election + recession + DebtGDP + ExpenditureGDP + PotentialGDP + DiscountRate2qChange + UNRATE + GlobalModel, model = "ls", data = subset(CPIMdfParty02, !(time_to_election %in% c(15, 14))), cite = FALSE)

PL.35.3 <- zelig(error.prop.deflator.q3 ~ pres_party + time_to_election + recession + DebtGDP + ExpenditureGDP + PotentialGDP + DiscountRate3qChange + UNRATE + GlobalModel, model = "ls", data = CPIMdfParty35, cite = FALSE)

PL.35.4 <- zelig(error.prop.deflator.q4 ~ pres_party + time_to_election + recession + DebtGDP + ExpenditureGDP + PotentialGDP + DiscountRate4qChange + UNRATE + GlobalModel, model = "ls", data = subset(CPIMdfParty35 , !(time_to_election %in% c(15, 14, 13, 12))), cite = FALSE)

PL.35.5 <- zelig(error.prop.deflator.q5 ~ pres_party + time_to_election + recession + DebtGDP + ExpenditureGDP + PotentialGDP + DiscountRate5qChange + UNRATE + GlobalModel, model = "ls", data = subset(CPIMdfParty35 , !(time_to_election %in% c(15, 14, 13, 12, 11))), cite = FALSE)
