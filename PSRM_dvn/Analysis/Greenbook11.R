####################
# Greenbook MatchIt Analyses Coef Plots Compare (Supplemental Materials Figure 4)
# Christopher Gandrud
# 14 October 2014
####################
...
CoefPlotPrep <- function(x){
    ...
    if (z == "L"){
        Lower.molten <- melt(Sum, id = c("var"), measure.vars = c("2.5 %"))
    } else if (z == "B"){
        Lower.molten <- melt(Sum, id = c("var"), measure.vars = c("2.5%"))
    }
    Lower.molten <- rename(Lower.molten, c(value = "lower"))
    Lower.molten <- Lower.molten[, -2]

    # Extract upper bounds
    if (z == "L"){
        Upper.molten <- melt(Sum, id = c("var"), measure.vars = c("97.5 %"))
    } else if (z == "B"){
        Upper.molten <- melt(Sum, id = c("var"), measure.vars = c("97.5%"))
    }
    Upper.molten <- rename(Upper.molten, c(value = "upper"))
