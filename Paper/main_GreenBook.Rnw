...
To examine these two possibilities we created parametric models with two-way and three-way interactions between presidential and congressional party identification.

The interactions were generally statistically significant. To make substantive sense of these estimates we use simulations, as above, to find expected inflation forecast errors at various levels of the Presidential and Congressional party identification.  
...
<<InterPlot, warning=FALSE, echo=FALSE, message=FALSE, results='hide', fig.width=6, fig.height=4, out.width='0.8\\linewidth'>>=
    ...
    InterURL <- "https://raw.github.com/christophergandrud/GreenBook/master/Analysis/PresCongressInteractions.R?login=christophergandrud&token=44bbeb1d649a3fd6e6d69ac4d842c3ed"
    source_url(InterURL)
...
@
    ...
    \end{center}
    \begin{singlespace}
        {\scriptsize{From an OLS regression with data matched by presidential party identification. Variables included are the same as those in Model C10 from Table \ref{OutputPL}. \\ The figures show 1000 simulations per fitted value. \\ Both the House and Senate Democratic/Republican variables were set at 1.2 for `Democratically controlled' congresses and 0.8 for `Republican controlled' congresses. \\ The solid line connects the means of the congressional partisan control groups.}}
    \end{singlespace}
\end{figure}
