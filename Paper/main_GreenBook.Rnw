...
%%%%%%% Abstract %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
\noindent\emph{Draft version. Comments welcome.}\footnote{Thank you to the Mark Hallerberg and the Hertie Fiscal Governance Centre for support.} \\[0.2cm]
...
Interestingly, staff do not seem to compensate for FOMC partisan biases in an attempt to moderate FOMC-driven partisan electoral business cycles.
...
The introduction of the FRB/Global behavioral equation forecasting model does not seem to have begun a different era inflation forecasting error. In fact, across all of our matching and parametric model specifications, forecasts made after the introduction of this approach are not significantly different than those made before it. 
...
Given the consistency across presidential terms, it appears that Fed staff members' partisan inflation forecast bias may be the result of a partisan heuristic. Like heuristics generally, the partisan bias may help Fed staffers simplify very complex phenomenon, while also to systematic prediction errors. These errors may not have been noticed by staffers because, so far, few if any people either inside or outside of the Fed have been looking for them. 

What are the monetary policy and electoral implications of Federal Reserve staff partisan bias? Our research so far is not able to tell. It does motivate future research and point this research in a clear direction to find out. It may be that higher inflation forecasts during Democratic presidencies spur the FOMC to raise interest rates and dampen the money supply generally. The opposite could happen during Republican presidencies. If this is the case, the economy would be inadvertently stimulated during Republican presidencies and depressed during Democratic ones, with clear electoral implications. This is a very important issue that needs further investigation.
...
This paper was written using {\tt{knitr}} \citep{knitr2012}. It can be entirely replicated from data, analysis source code, and markup files available as a {\tt{.zip}} file downloadable from our GitHub page at: {\url{https://github.com/christophergandrud/GreenBook}}. 
...
<< ELTables, warning=FALSE, echo=FALSE, message=FALSE, results='asis'>>=
    ...
    CoefNamesB <- c("Intercept", "Debt/GDP", "Expenditure/GDP", "Output Gap", "Discount Rate Change", "Qtr. to Election", "Election Period", "Pres. Party ID", "Pres*Election Period", "Senate Dem/Rep", "House Dem/Rep", "FRB/GlobalModel", "Pres*House", "Pres*Senate", "House*Senate", "Pres*House*Senate") 
...
<< PLTables, warning=FALSE, echo=FALSE, message=FALSE, results='asis'>>=
    ...
    CoefNamesC <- c("Intercept", "Recession", "Debt/GDP", "Expenditure/GDP", "Output Gap", "Discount Rate Change", "Qtr. to Election", "Election Period", "Pres. Party ID","Pres*Election Period", "Senate Dem/Rep", "House Dem/Rep", "FRB/GlobalModel", "Pres*House", "Pres*Senate", "House*Senate", "Pres*House*Senate") 
