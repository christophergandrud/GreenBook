...
We have 36 election period `treatments' in our data set. Our data set matched by election period only contained about 57 observations. Because of this small sample size, we do not show estimates from models using data matched on election period.\footnote{More information on the models can be found at \url{http://bit.ly/OFdA4u}.}
