...
We examined the monetary policy and partisan preference theories of forecast errors with an interaction between the president's party ID variable and the time to election variable. The president's party ID variable is robust whereas the election variable and the interaction term are very statistically insignificant in all models where it is included. This is reflected in the left-side graph in Figure \ref{InterPlot}.\footnote{This plot shows simulations from unmatched data. The results from models with matched data are virtually identical.} Inflation underestimates are predicted to become even worse as an election nears during Republican presidencies and slightly more overestimated close to elections for Democrats as the partisan preferences theory predicted. However it is important to note that this is not a statistically significant effect.

Of course, Figure \ref{ExpectGraphs} predicts that the observed trajectory of the errors if the partisan preferences may be non-linear, i.e. errors become much worse near elections. However, when we included a number of non-linear transformations of the time to election variable, the results did not change substantively.
...
<< ELTables, warning=FALSE, echo=FALSE, message=FALSE, results='asis'>>=
    ...
    CoefNamesB <- c("Intercept", "Debt/GDP", "Expenditure/GDP", "Output Gap", "Discount Rate Change", "Qtr. to Election", "Election Period", "Pres. Party ID", "Senate Dem/Rep", "House Dem/Rep", "FRB/GlobalModel", "Qrt. Election2", "Pres*Qrt. Election2", "Pres*House", "Pres*Senate", "House*Senate", "Pres*House*Senate") 
...
<< PLTables, warning=FALSE, echo=FALSE, message=FALSE, results='asis'>>=
    ...
    CoefNamesC <- c("Intercept", "Recession", "Debt/GDP", "Expenditure/GDP", "Output Gap", "Discount Rate Change", "Qtr. to Election", "Election Period", "Pres. Party ID", "Senate Dem/Rep", "House Dem/Rep", "FRB/GlobalModel", "Qrt. Election2", "Pres*Qrt. Election2", "Pres*House", "Pres*Senate", "House*Senate", "Pres*House*Senate") 
