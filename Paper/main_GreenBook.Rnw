...
In this section we present results from a number of parametric model specifications. We only have 18 election period `treatments' in our data set. So, unfortunately our data set matched based on the election period only contained about 30 observations. Because of this very small sample size, we do not show estimates from models using data matched on election period. \footnote{More information on the models can be found at \url{http://bit.ly/OFdA4u}.}
